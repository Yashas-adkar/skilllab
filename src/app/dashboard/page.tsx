...
function DashboardContent() {
  ...
  const { stageProgress } = useInterviewSession();
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 transition-colors duration-150">
      {/* Welcome Banner */}
      <div className="relative overflow-hidden rounded-3xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/80 p-6 sm:p-8 shadow-sm dark:shadow-none">
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2 max-w-2xl">
            ...
            <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed">
              Ready to prepare for your next interview? The platform contains four progressive preparation stages designed to measure and elevate your readiness for <strong className="text-slate-900 dark:text-white">{stream.shortName}</strong> roles.
            </p>
          </div>
      ...
      {isAllComplete && (
        <div className="p-6 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-500/40 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-4 animate-fadeIn shadow-sm dark:shadow-xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/20 text-indigo-600 dark:text-indigo-400 flex items-center justify-center shrink-0">
            <Award className="w-6 h-6" />
          </div>
          <div>
            <div className="text-xs text-slate-500 dark:text-slate-400 font-medium">Preparation Progress</div>
        ...
        <Card className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 text-emerald-600 dark:text-emerald-400 flex items-center justify-center shrink-0">
            <CheckCircle2 className="w-6 h-6" />
          </div>
          <div>
            <div className="text-xs text-slate-500 dark:text-slate-400 font-medium">Pipeline Progression</div>
            <div className="text-2xl font-bold text-slate-900 dark:text-white mt-0.5">{completedCount} / 4</div>
            <div className="text-[11px] text-slate-500">Sequential advancement</div>
          </div>
        </Card>

        <Card className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-purple-50 dark:bg-purple-500/10 border border-purple-200 dark:border-purple-500/20 text-purple-600 dark:text-purple-400 flex items-center justify-center shrink-0">
            <Layers className="w-6 h-6" />
          </div>
          <div>
            <div className="text-xs text-slate-500 dark:text-slate-400 font-medium">Active Track</div>
            <div className="text-lg font-bold text-slate-900 dark:text-white mt-0.5 truncate max-w-[160px]">
              {stream.shortName}
            </div>
            ...
            return (
              ...
              >
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] font-bold tracking-wider uppercase px-2 py-0.5 rounded-md bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400">
                  ...
                  <div className="space-y-1.5">
                    <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/60 text-indigo-600 dark:text-indigo-400 flex items-center justify-center">
                      {stageIcons[stage.iconName]}
                    </div>
                    <h3 className="font-bold text-base text-slate-900 dark:text-white">{stage.title}</h3>
                    <p className="text-xs font-medium text-indigo-600 dark:text-indigo-400">{stage.subtitle}</p>
                  </div>
